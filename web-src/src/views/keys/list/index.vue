<template>
  <div class="container-list">
    <Breadcrumb :items="['menu.keys', 'menu.keys.list']"/>
    <div class="contain">
        ...
        <div class="tiny-fullscreen-scroll">
          <div class="tiny-fullscreen-wrapper">
            ...
            >
                    ...
                    <div class="btn">
                      ...
                      <div class="screen">
                        ...
                        <span @click="toggle">
                          {{
                            fullscreen
                                ? $t( 'keys.collapse.restores' )
                                : $t( 'keys.collapse.full' )
                          }}
                        </span>
                      </div>
                    </div>
              ...
              <tiny-grid-column
                  field="keyname"
                  :title="$t('keys.columns.keyname')"
                  align="center"
              >
            </tiny-grid-column>
            <tiny-grid-column
                  field="publickey"
                  :title="$t('keys.columns.publickey')"
                  align="center"
              >
                <template #default>
                    <tiny-tag >数据安全不显示</tiny-tag>
                </template>
              </tiny-grid-column>
              <tiny-grid-column
                  field="privatekey"
                  :title="$t('keys.columns.privatekey')"
                  align="center"
              >
                <template #default>
                  <tiny-tag >数据安全不显示</tiny-tag>
              ...
              <tiny-grid-column
                  :title="$t('keys.columns.operations')"
                  align="center"
              >
                <template  v-slot="data">
                  <a class="operation-item" @click="handleDelete(data.row.id)">
                    {{ $t('keys.columns.operations.delete') }}
                  </a>
                </template>
              </tiny-grid-column>
            </tiny-grid>
...
import type {QueryParmas} from '@/api/keys';
...
async function fetchData(
  ...
  try {
    ...
    const res:any = await queryKeysList(queryParmas);
